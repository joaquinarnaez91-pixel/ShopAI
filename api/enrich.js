...
export default async function handler(req, res) {
  ...
  const fallback = () => res.status(200).json(
    products.map(p => ({ ...p, summary: '✅ Highly rated by verified buyers.', insight: p.snippet, funFact: '' }))
  );
      ...
      apiRes.on('end', () => {
        try {
          ...
          res.status(200).json(products.map((p, i) => ({
            ...
            funFact: enriched[i] ? enriched[i].funFact || '' : ''
          })));
        } catch(e) { fallback(); }
      });
    });
