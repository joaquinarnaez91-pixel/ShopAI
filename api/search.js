...
async function wardrobeSimilar(name, brand, category, colors, serpKey, anthropicKey) {
  const garmentDesc = [name, brand && `by ${brand}`, category, colors && `colors: ${colors}`]
    .filter(Boolean).join(', ');
  const claudePayload = JSON.stringify({
    model: 'claude-sonnet-4-6',
    max_tokens: 150,
    messages: [{ role: 'user', content: `You are a fashion shopping expert. Given this owned garment, write 1 concise Google Shopping search query to find visually similar items, plus 2 alternates at different price points. Return JSON only: {"queries":["...","...","..."]}\n\nGarment: ${garmentDesc}` }]
  });
  let queries = [];
  try {
  ...
  if (!queries.length) queries = [`${brand ? brand + ' ' : ''}${name}`.trim(), `similar ${category || 'clothing'} ${name}`, `affordable ${name}`];
  ...
  for (const q of queries) {
    ...
    try {
      for (const p of await serpSearch(q, serpKey, 10)) {
        if (!seen.has(p.link)) { seen.add(p.link); results.push(p); }
      }
...
export default async function handler(req, res) {
  ...
  // Wardrobe-similar mode: body has {name} but no {models}
  if (req.body?.name && !req.body?.models) {
    const { name, brand, category, colors } = req.body;
    const products = await wardrobeSimilar(name, brand || '', category || '', colors || '', serpKey, process.env.ANTHROPIC_API_KEY);
